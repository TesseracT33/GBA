...
namespace Logging
{
	...
	export void LogInstruction(u32 pc, u32 opcode, std::string_view cond_str, bool cond, const std::array<u32, 16>& r)
	{
		if (!log.is_open()) {
			return;
		}
		log << std::format("{:08X}  ARM:{:08X}  cond:{} ({}) {}\n", pc, opcode, cond_str, cond, FormatRegisters(r));
	}

	export void LogInstruction(u32 pc, u16 opcode, const std::array<u32, 16>& r)
	{
		if (!log.is_open()) {
			return;
		}
		log << std::format("{:08X}  THUMB:{:04X} {}\n", pc, opcode, FormatRegisters(r));
	}

	export void LogInstruction(u32 pc, std::string instr_output, const std::array<u32, 16>& r)
	{
		if (!log.is_open()) {
			return;
		}
		log << std::format("{:08X}  {} {}\n", pc, instr_output, FormatRegisters(r));
	}
}